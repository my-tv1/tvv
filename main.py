...
def updateChannelUrlsM3U(channels, template_channels):
   ...
   # for group in config.announcements:
    ...
    with open("iptv.m3u", "w", encoding="utf-8") as f_m3u:
                    ...
                    for channel_name in channel_list:
                        if channel_name in channels[category]:
                            ...
                            for url in sorted_urls:
                                if url and url not in written_urls and all(blacklist not in url for blacklist in config.url_blacklist) and any(whitelist in url for whitelist in config.url_whitelist):
